...
function JobData() {
  return (
    <div className="w-full h-fit bg-cream flex flex-col lg:justify-around py-8 lg:py-12 gap-10 lg:flex-row ">
      {STATIC_DATA.map((stat, id) => (
        <div className="flex flex-col items-center justify-center gap-4 lg:flex-row " key={id}>
          <span
          ...
          <div className="stats-content flex flex-col items-center text-gray lg:items-start">
            ...
            <span className="uppercase text-sm font-semibold">{stat.title}</span>
          </div>
        </div>
      ))}
    </div>
